...
public record ConditionalLoadable<T extends IHaveLoader>(GenericLoaderRegistry<T> registry, @Nullable T defaultIfFalse) implements RecordLoadable<T> {
  public static boolean processConditions(JsonObject json, IContext conditionContext) {
    String key = json.has("neoforge:conditions") ? "neoforge:conditions" : (json.has("conditions") ? "conditions" : null);
    if (key == null) {
      return true;
    }
    List<ICondition> conditions = ICondition.LIST_CODEC.parse(JsonOps.INSTANCE, json.get(key)).result().orElse(List.of());
