...
public interface ICommonRecipeHelper extends IRecipeHelper {
  ...
  static Criterion<InventoryChangeTrigger.TriggerInstance> has(ItemLike item) {
    return CriteriaTriggers.INVENTORY_CHANGED.createCriterion(new InventoryChangeTrigger.TriggerInstance(
        ...
        List.of(ItemPredicate.Builder.item().of(item).build())
    ));
  }
  ...
  static Criterion<InventoryChangeTrigger.TriggerInstance> has(TagKey<Item> tag) {
    return CriteriaTriggers.INVENTORY_CHANGED.createCriterion(new InventoryChangeTrigger.TriggerInstance(
        Optional.empty(),
        InventoryChangeTrigger.TriggerInstance.Slots.ANY,
        List.of(ItemPredicate.Builder.item().of(tag).build())
    ));
  }

  static Criterion<InventoryChangeTrigger.TriggerInstance> inventoryTrigger(ItemPredicate.Builder... predicates) {
    return CriteriaTriggers.INVENTORY_CHANGED.createCriterion(new InventoryChangeTrigger.TriggerInstance(
        Optional.empty(),
        InventoryChangeTrigger.TriggerInstance.Slots.ANY,
        Arrays.stream(predicates).map(ItemPredicate.Builder::build).toList()
    ));
  }
  ...
  default void metalCrafting(RecipeOutput consumer, MetalItemObject metal, String folder) {
    ...
    packingRecipe(consumer, RecipeCategory.BUILDING_BLOCKS, "block", metal.get(), "ingot", ingot, folder);
    packingRecipe(consumer, RecipeCategory.MISC, "ingot", ingot, "nugget", metal.getNugget(), folder);
  }
   ...
   * @param folder    Recipe folder
   */
  default void packedMetalCrafting(RecipeOutput consumer, MetalItemObject metal, String folder) {
    ItemLike ingot = metal.getIngot();
