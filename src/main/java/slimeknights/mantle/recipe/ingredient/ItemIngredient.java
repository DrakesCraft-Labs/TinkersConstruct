...
public abstract class ItemIngredient implements ICustomIngredient {
  ...
  public Stream<ItemStack> getItems() {
    Stream<ItemStack> itemStream = items.stream().map(ItemStack::new);
    if (tag != null) {
      Stream<ItemStack> tagStream = BuiltInRegistries.ITEM.getTag(tag)
        .map(named -> named.stream().map(Holder::value).map(ItemStack::new))
