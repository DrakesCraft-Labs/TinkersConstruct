...
public class OffhandCooldownTracker {
  ...
  public static final DeferredRegister<AttachmentType<?>> ATTACHMENT_TYPES = DeferredRegister.create(NeoForgeRegistries.ATTACHMENT_TYPES, Mantle.modId);
  public static final Supplier<AttachmentType<OffhandCooldownTracker>> ATTACHMENT = ATTACHMENT_TYPES.register("offhand_cooldown", () ->
    AttachmentType.builder((IAttachmentHolder holder) -> new OffhandCooldownTracker(holder instanceof Player p ? p : null)).build()
  );
  ...
  public static void register(RegisterCapabilitiesEvent event) {
    event.registerEntity(CAPABILITY, EntityType.PLAYER, (player, ctx) -> player.getData(ATTACHMENT.get()));
  }

  public static void init() {
    // No-op in 1.21; data attachments handle lifecycle automatically
  }
