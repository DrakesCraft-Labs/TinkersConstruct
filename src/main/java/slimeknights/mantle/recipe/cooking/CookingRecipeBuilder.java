...
public class CookingRecipeBuilder<T extends CookingRecipeBuilder<T>> extends AbstractRecipeBuilder<T> {
  ...
  @FunctionalInterface
  public interface CookingRecipeFactory<R extends Recipe<?>> {
    R create(String group, CookingBookCategory category, Ingredient ingredient, ItemOutput result, float experience, int cookingTime);
  }

  /** Helper to save a recipe */
  @SuppressWarnings("unchecked")
  private <R extends Recipe<?>> T save(RecipeOutput output, ResourceLocation id, RecordLoadable<R> loadable, CookingRecipeFactory<R> constructor, int cookingTime) {
    if (ingredient == Ingredient.EMPTY) {
      throw new IllegalStateException("Ingredient must be set");
    }
    AdvancementHolder advancement = buildOptionalAdvancement(output, id, "cooking");
    output.accept(id, constructor.create(group, category, ingredient, result, experience, cookingTime), advancement);
    return (T) this;
  }
