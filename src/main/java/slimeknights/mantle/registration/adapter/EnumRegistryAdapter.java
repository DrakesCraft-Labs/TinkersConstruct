...
public class EnumRegistryAdapter<T> extends RegistryAdapter<T> {
   ...
   * @param mapper    Function to get a block for the given enum value
   * @param values    Enum values to use for this block
   * @param prefix    Prefix for the block name
   * @param postfix   Suffix for the block name
   * @return  EnumObject mapping between different block types
   */
  public <E extends Enum<E> & StringRepresentable,I extends T> EnumObject<E,I> registerEnum(Function<E,I> mapper, E[] values, String prefix, String postfix) {
    return registerEnum(mapper, values, prefix + "_" + postfix);
  }
}
