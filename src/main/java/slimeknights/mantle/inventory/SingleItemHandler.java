...
public abstract class SingleItemHandler<T extends MantleBlockEntity> implements IItemHandlerModifiable {
   ...
   * @param registries HolderLookup provider
  ...
  /**
   * Writes this module to NBT using parent's level registry access if available
   * @return  Module in NBT
   */
  public CompoundTag writeToNBT() {
    return parent.getLevel() != null ? writeToNBT(parent.getLevel().registryAccess()) : new CompoundTag();
  }
  ...
  /**
   * Reads this module from NBT using parent's level registry access if available
   * @param nbt  NBT
   */
  public void readFromNBT(CompoundTag nbt) {
    if (parent.getLevel() != null) {
      readFromNBT(parent.getLevel().registryAccess(), nbt);
    }
  }
}
